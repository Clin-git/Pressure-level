...
{
    __HAL_TIM_SET_AUTORELOAD(&htim3, (HAL_RCC_GetPCLK1Freq() / period) - 1);
    __HAL_TIM_SET_COMPARE(&htim3, channel, (HAL_RCC_GetPCLK1Freq() / period) * pulse / period);
}

/**@brief       读取 TIM3 PWM 周期和脉宽
* @param[in]    channel : 指定 TIM 通道
* @param[in]    period : PWM频率缓存
* @param[in]    pulse : 占空比缓存
* @return       函数执行结果
* - None
*/
static void TIM3_PWM_Get(uint32_t channel, uint32_t *period, uint32_t *pulse)
{
    if(period != NULL)
    {
        *period = HAL_RCC_GetPCLK1Freq() / (__HAL_TIM_GET_AUTORELOAD(&htim3) + 1);
    }
    if(period != NULL && pulse != NULL)
    {
        *pulse = (__HAL_TIM_GET_COMPARE(&htim3, channel) * (*period * *period)) / HAL_RCC_GetPCLK1Freq();
    }
}
...
void TIM_DevObj_Init(void)
{
    ...
    tim_dev[TIM3_INDEX].tim_get_pwm     = TIM3_PWM_Get;
#endif // BSP_USING_TIM3
